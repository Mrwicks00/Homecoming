...
// Vendored verbatim, unmodified, from cowprotocol/hooks-trampoline
// (https://github.com/cowprotocol/hooks-trampoline/blob/main/src/HooksTrampoline.sol), included so
// Homecoming's Ethereum Sepolia deployment does not depend on locating or trusting a third-party
// shared instance — anyone may deploy their own copy pointed at the real GPv2Settlement, since the
// contract itself is stateless beyond its immutable `settlement` address and has no privileged
// role. See DeployCowLeg.s.sol and MECHANISM.md §5.
